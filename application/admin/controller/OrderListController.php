...
class OrderListController extends Controller
{
    ...
    {
        $user = new WayLog();//实例化model

       /* $res = $user->order('is_pay asc')->paginate(9);*/
        $res      = array();
        $car_num  = ""; 
        $username = "";
        $is_Print = "";

        $res = Db::name('WayLog')->order('is_pay asc')->paginate(9);

        $res_userbindcar = Db::name('way_user_bind_car')->select();

        $res_invoice = Db::name('WayInvoice')->select();

        $username = $res_userbindcar[0]['username'];
        $car_num = $res_userbindcar[0]['car_number'];
        $is_Print = $res_invoice[0]['is_Print'];
        
        if(empty($_POST['contents'])&&$res['user_id']==$res_userbindcar['user_id']==$res_invoice['user_id'])
        {
    ...
    public function insertAction()
    {
        ...
        {   
            ...
            //echo "<script>alert('新增成功');localtion.href='{:url('Index/orderlist')}'</script>";
            $this->success('新增成功','UserList/orderlist');
        }else{
            $this->error('新增失败');
        }
